...
def _selftest_search_server():
    """Confirm the loopback ARTGEOrge server is up and serving the 9-dot home
    page before the UI loads it. Prints the result so launching from a Terminal
    surfaces any problem (e.g. the page the browser will actually show)."""
    import urllib.request
    try:
        with urllib.request.urlopen(HOME_URL, timeout=4) as r:
            html = r.read().decode("utf-8", "replace")
        ok = ("appsbtn" in html) and ("ARTGEOrge" in html)
